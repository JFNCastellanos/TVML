...
class TestOperators():
    def __init__(self,nv,nx,nt,blocks_x,blocks_t):
        ...
        self.P = torch.zeros(self.n,self.nv*self.na,dtype=var.PREC_COMPLEX,device=var.DEVICE)
        self.Pdagg = torch.zeros(self.nv*self.na,self.n,dtype=var.PREC_COMPLEX,device=var.DEVICE)
    
    def flatten_col(self,col):
        flat = torch.zeros(self.n,dtype=var.PREC_COMPLEX,device=var.DEVICE)
    ...
    def flatten_colV2(self,col):
        flat = torch.zeros(self.nv*self.na,dtype=var.PREC_COMPLEX,device=var.DEVICE)
        ...
        for tv in range(self.nv):
            for bx in range(self.blocks_x):
                for bt in range(self.blocks_t):
                    for s in range(2):
                        vc = torch.zeros((self.nv,2,self.blocks_t,self.blocks_x),dtype=var.PREC_COMPLEX,device=var.DEVICE)
                        vc[tv,s,bt,bx] = 1
    ...
    def assemblePdagg(self):
        for x in range(self.nx):
            for t in range(self.nt):
                for s in range(2):   
                    v = torch.zeros((2,self.nt,self.nx),dtype=var.PREC_COMPLEX,device=var.DEVICE)
                    v[s,t,x] = 1
