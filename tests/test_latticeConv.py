...
class TestLCNN():
    ...
    def __init__(self,batchsize):
        ...
        confID = 0 
        self.data = self.first_batch[confID].to(var.DEVICE) #[Re(U0),Re(U1),Im(U0),Im(U1), Re(W),Im(W)], where W'(x) = Omega(x) W(x) Omega^+(x)
        print(len(self.data))
    ...
    def check_parallel_transport(self):
        ...
        wkxmu = torch.roll(self.w,shifts=k*orientation,dims=mu)   #w[(t,x)+k * orientation *hat{mu}]

        #assert self.w[0,0,0] == wkxmu[0,k*mu,0], "W(0,0) = {0} and W(k mu,0) = {1}".format(self.w[0,0,0],wkxmu[0,k*mu,0])
        print("W(0,0)={0}, W(-kmu,0)={1}".format(self.w[:,0,0],wkxmu[:,-k*mu,0]))
        print("W(0,0)={0}, W(kmu,0)={1}".format(self.w[:,0,0],wkxmu[:,k*mu,0]))
        print("W(0,0)={0}, W(0,-kmu)={1}".format(self.w[:,0,0],wkxmu[:,0,-k*mu]))
        print("W(0,0)={0}, W(0,kmu)={1}".format(self.w[:,0,0],wkxmu[:,0,k*mu]))